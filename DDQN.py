...
class Actor:
    def get_action(self, state, episode, targetQN):   # [C]ｔ＋１での行動を返す
        ...
        epsilon = 0.00025 + 0.9 / (1.0+episode/10)
        # epsilon = 0.00025
...
isrender = 0  # 描画フラグ
# ---
hidden_size = 12               # Q-networkの隠れ層のニューロンの数
learning_rate = 0.0003         # Q-networkの学習係数
memory_size = 30000            # バッファーメモリの大きさ
...
for episode in range(num_episodes):  # 試行数分繰り返す
    ...
    for t in range(max_number_of_steps + 1):  # 1試行のループ
        ...
        episode_reward += reward # reward  # 合計報酬を更新
        ...
        if (memory.len() > batch_size) and (t%1 == 0):
        ...
        if done:
            ...
            print('%d Episode finished after %f time steps / mean %f' % (episode, t + 2, total_reward_vec.mean()))
            break
...
for i_episode in range(3):
    observation = env.reset()
    state = observation.reshape(1,2)
    for t in range(200):
        env.render()
        print(observation)
        action = action = actor.get_action(state, 1e+30, mainQN)
